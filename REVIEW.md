...
要么只写命令不写输出（ADR-73、ADR-82）—— 但**时间数不是树的函数**，钉了树也复现
不出来（ADR-99 第一节：同一棵树的全链墙钟跑出过 603 s 和 788 s），计时要给抗抢占的
量（CPU 时间）与多次的区间；一句承诺往往有多份副本，改一处要点名逐个过、
不要 grep（`docs/SYNC.md`）。
