...
export interface Creator {
   ...
   * 所以读它的地方一律把空数组和 `undefined` 当成同一态（P1.i）；「他确实没作品」
   * 只有主页样本测过、确认为空这一种证据给得出。
   */
...
export interface TaskState {
   ...
   * 否则会把已经付过费的几页重新抓一遍。
   *
   * **它同时是「这个任务抓过第一页没有」的记录（F9）**，所以三态都有意义：
   * 整张表缺失 = 无从确认（F9 落地之前的旧任务目录）；
   * 有表但没有这个键 = 这个任务一页都没抓过；
   * 有值（含 0）= 抓过，0 是「抓了但一条没返回」。
   * **可选不是手滑** —— 旧目录里真的没有这张表，而把「没有表」读成「都没查过」
   * 会把已经付过费的词重抓一遍。
   *
   * ⚠️ **它只回答「下一页从哪儿接」**。一度拿它兼职回答「查过没有」与「查到几条」，
   * 独立复核推翻了那个设计（ADR-94 第十五节甲，含两处实测反例）。那两件事各有自己的表
   * （`answered` 与 `found`）。
   */
   ...
   * 上一版留下的目录 —— 那种目录此后也不长出这两张表，一直说「无从确认」。
   * 凭空建一张空表就是把「无从确认」抹成「一个都没问过」，不可逆。
   */
  answered?: Record<number, number>

  /**
   * D6.l：这个任务累计拿回来多少**条**（供应商返回的条目，不是人数）。
   *
   * **值可以是 `null` ＝「这个数注定不全」**：付了钱、却在记下条数之前抛了（供应商换了
   * 响应结构、兜底撞上预算），那一次的条数永远补不回来。不记 `null` 的话，下一页成功时
   * 它会从 0 重新数起，凑出一个偏小、却和真测量值印在同一列的数 —— 读的人拿它当分母。
   * **所以一旦不全就永远是 `null`。**
   *
   * 三态与建表时机同 `answered`。
   */
  found?: Record<number, number | null>

  /**
   * D6.h：这个任务**累计真的拿回了几页** —— 跨运行累加，页数上限据它判。
   *
   * 与旁边那两张表都答不了同一个问题：`answered` 记的是**发出过几次付费请求**，
   * Instagram 兜底一页要发两次（D6.k），拿它当页数会把 IG 的配额腰斩；`found` 记的是
   * **条数**，而每页多少条本来就不固定。所以这个数只能单独记。
   *
   * **只在真的拿回了一页时 +1**（与 `offsets` 同一处落笔、同一份证据：请求正常返回）。
   * 付了钱却没拿回页的那几次由 `answered` 记、不占页数配额 —— 记进去的话，一个供应商
   * 换了响应结构的词会被「重试次数」耗光配额，而它一个人都没采到。
   *
   * ⚠️ 值**不需要** `null` 那一态（`found` 需要，因为它要被印进关键词表当测量值给人看）。
   * 这个数一个字都不进交付物，它只喂一道闸，少记一页的后果是**多给一页** ——
   * 方向已知、有界，不会伪装成测量值。
   *
   * ⚠️ **整张表缺失＝无从确认**，建表时机与缺表读法在 D6.m，判定只此一份：
   * `pagesFetched`（`lib/pipeline.ts`）。
   */
