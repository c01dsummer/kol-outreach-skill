...
export interface TaskState {
  ...
  /**
   * D6.i：这个任务让供应商**应答过几次**（200 且计过费）。「从未发出过搜索请求」
   * 只能由它回答，而且**写在 finally 里** —— 钱扣了、在记录之前抛了也要留下痕迹。
   *
   * ⚠️ **非 200 不计入**：402／429 会 `refund()`，钱没花也没拿回东西，而续跑会重试它 ——
   * 读作「还没查过」既如实也可行动。这是取舍，不是 bug。
   *
   * 三态同 `offsets`：整张表缺失＝无从确认（本条落地之前的目录）。
   */
  answered?: Record<number, number>

  /**
   * D6.i：这个任务累计拿回来多少**条**（供应商返回的条目，不是人数）。
   *
   * 与 `offsets` 分开：两者不恒等（IG 兜底那条路上 offsets 只记最后一个端点）。
   * 三态同上；**有 `answered` 没有 `found`** ＝问过、条数没记下，渲染成「未知」，不许写 0。
   *
   * **值可以是 `null` ＝「这个数注定不全」**：升级前抓了几页无从得知，再往上加只会凑出
   * 一个偏小却像测量值的数，运营拿它当分母 —— 所以一旦不全就**永远**是 `null`。
   */
