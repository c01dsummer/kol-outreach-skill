...
suite('D6', '「还要不要补 profile」只有一个标准 —— 挨个问和问一整份名单，答的是同一批人')
{
  ...
  criterion('D6.l')
  // 上面验的是这个标准本身，以及问一个人和问一整份名单答得一样。**入口那一侧不在这里**：
  ...
  // 三条判据和它们各自的假阳性记在 ADR-62。端到端那一半在 `scripts/check/selfcheck.ts`
  // 的「查过了但没外链的人」那一段：罐头数据里单挂了这么一个人（`fake-fetch.ts` 的
  // nolink 两份），跑完再续跑一次 —— 续跑一个请求都没多发，就说明他被漏在补全循环外面。
}
...
suite('D6', '收尾说哪一句是算出来的 —— 挑错分支，用户就被告知续跑免费')
{
  ...
  // 花钱 —— 少哪一半，用户都拿不到「要不要续跑」这个决定所需的东西（负片 M-D6-n 只
  // 掐掉「还剩什么」那一半，别的照旧）。
  ...
  // （M-D6-o 只掐掉「续跑不产生新的请求」，上面四条照旧全绿），所以另算一条判据。
