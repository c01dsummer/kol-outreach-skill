 ...
 * | 已合并 PR #8–#15 | 1–4 | 0.1–2.2 小时 |
 * | 已合并 PR #6 / #7 | 34 / 29 | 22.9 / 5.4 小时 |
 * | **在途** `durable-persistence` | 21 | **91.5 小时** |
 * | **在途** `requirement-conflict` | 23 | **91.5 小时** |
 * | **在途** `requirements-registry` | 18 | **102.8 小时** |
 *
 * 48 小时对三条出事的分支只有一半 —— 这条线不是拍的,是它们与已合并分支之间的空档。
 *
 * ⚠️ **48 小时是红线,不是目标。** 早先这里把 #6 列为「最长的健康分支」,按它留一倍余量。
 *    ADR-64 复盘之后 #6 / #7 归到另一类:分叉时长在线内,却各自吞了 22 / 19 轮评审,
 *    源码长到开 PR 时的近三倍。分叉时长在线内、评审也没让它长大的那一组是 #1–#4 与 #8–#15,
 *    全部在 2.2 小时内合入 —— 但这只说明分叉时长,不说明评审干净:#10、#12 是先合再审
 *    (合并早于第一条评审意见),#10 让主干带着一个静默错跑了一个多小时,由 #14 修掉。
 *    拿这张表重新校准阈值时,那两条不能当健康样本。
 *    这条闸门量不到「评审中长了多少」,那在 `process/6-INTEGRATE.md`「评审中的发现」
 *    一节,第三层;阈值要不要收到几小时,等那一节跑出数据再另立记录,不在这里拍。
 *
