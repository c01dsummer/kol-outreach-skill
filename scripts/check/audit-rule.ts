/**
 * 链路审计里能被测的那些判定 —— 抽出来的理由同 `size-rule.ts`:留在入口里就永远测不到
 ...
 *
 * 这个文件自己也在名单上 —— 谁守着它,跑一次 `npm run audit` 会逐条印出来。
 * 少了这一步,这条检查就是它自己要拦的那种东西。
 *
 * 死亡条件记在 ADR-85:关系钩那两节各一条;红线覆盖那两节判过,不该有。
 */
 ...
 * 变异里**记在判据名下**的那些编号。
 *
 * `req` 这一栏两种编号混着写 —— 带点的是判据号、不带点的是需求号(分法就是下面那句
 * `id.includes('.')`) —— 而审计的「变异」那一列只拿需求号去查。判据号那几条因此
 * **对报告完全不可见**,一个字都没露过。是哪几条、各有多少种,不写在这儿(会漂,
 * 而且没有任何检查看着这段注释),命令见 ADR-82。
 *
 ...
 * 判据数两遍,红线那三栏加出**比红线判据总数还大**的值(落地 3 第二片实测撞见:
 * P3.b 拿到入口认领之后被「入口认领」和「显式豁免」各数了一遍)。总数每次
 * `npm run audit` 自己会印,不写在这儿。
 *
...
export function coverageSummary(
  ...
  // 「入口认领 1 · 显式豁免 2」把它数了两遍,红线那三栏加出比总数还大的值 ——
