 ...
 * 覆盖记录那一份**方向要说准**(评审指出,后来实测纠正):入口把它的还原挂在退出上,
 * 而按**当前这套变异集**,还原之后子进程只可能把它再**清掉**一次,写不回一份新的 ——
 * 动 `claimsOwnedBy` 的只有 M-H14-e 一条,而它自己会被断言抓到、`fail` 不为零,
 * 写盘资格那一半仍要求 `fail === 0`;另外两条(M-H14-d、M-H14-k)只削弱写盘资格,
 * `claimsOwnedBy(true)` 照样是 false。所以坏法是**假的「没测过」**(审计报「先跑
 * npm test」),不是一份被审计当真的假证据 —— 前者是响的,后者才是静默的。
 *
