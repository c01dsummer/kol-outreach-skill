 ...
 *
 * ⚠️ **尤其不要指望文件名撞车。** 标题不同文件名就不同,两份同号记录是各自干净
 * 落地的。文件名真撞只发生在标题也一样、或者两个标题经 `fileNameOf` 截断之后
 * 撞了的时候(截到 32 字符,现存记录里已有顶到上限的)—— 而那一格 git 报的是
 * 「两边都新建了这个文件」,人按「留一个」解掉就无声少了一条记录,这条检查事后
 * 只看得见一个文件、一路全绿。**那一格不是保护,是丢数据。**
 *
