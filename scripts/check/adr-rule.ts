 ...
 * 作为不同文件留下来的那一种**;同名那一种够不着,见 `checkAppendOnly` 上面那段。
 */
 ...
 * 而合并之前修改自己的草稿是正常的。**编号跨分支没有任何机制守着** —— 这条检查和
 * 索引那道合并冲突都只看得见眼前这棵树,别的分支上占着的号看不见;要等两边合到
 * 一起,才由 `checkAll` 比出「两个文件占同一个编号」—— 而这句成立的前提是**两份记录
 * 各自作为不同文件留了下来**。同名、又被「留一个」解掉的那一种,合出来的树上只有
 * 一个文件,`checkAll` 一路全绿(紧接着下面那段说的就是它)。
 *
 * ⚠️ **尤其不要指望文件名撞车。** 撞不撞取决于两个标题过完 `slugify` 之后一不一样,
 * 而 `slugify` 有损三步(剔标点、连续空白归一成 `-`、截到 32 码点),于是它**两头
 * 都不保证**:多数时候两份同号记录各自干净落地,什么也没挡住;而两个不同标题只要
 * 过完这三步一样,就照样同名。真撞的那一格更糟 —— git 报的是「两边都新建了这个
 * 文件」,人按「留一个」解掉就无声少了一条记录,这条检查事后只看得见一个文件、
 * 一路全绿。**那一格不是保护,是丢数据。**
 *
