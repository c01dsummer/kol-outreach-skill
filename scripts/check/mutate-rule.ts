...
export const VERIFIERS: Record<string, Verifier> = {
  ...
  selfcheck: {
    ...
    summary: /(^|\n)✗ 脚本自检：\d+ 项失败\s*(\n|$)/,
    subsetSuccess: /(^|\n)✓ 脚本自检（只跑 [1-9]\d* 组）：点名的那几组都跑完了，一条断言都没红\s*(\n|$)/,
 ...
 * 编译期都过不去),而那张表同时是隔离判据的种子来源:闭包会跟着撑大
 * (多收 `audit.ts` 自己,带上 `audit-rule` / `spec-rule` / `quoted`),此后能被自检验证的变异空间
 ...
 *
 * 重名只在**被点到**时才拦,不做全局唯一:`scripts/test.ts` 今天就有几个名字各起了
 * 不止一次,而谁也没点它们(是哪几个,用本文件的 `labelsOf` 建出清册、数计数大于 1 的)。全局唯一要为一条没人用到的性质去改一批无关的断言;
 ...
 *
 * `kills` 没写时不判第四态 —— 缺省跑测试的那些一条夹具也没点名,行为逐字如旧。
 *
...
export function judgeRun(exitCode: number | null, output: string,
  ...
  // 比「打出了汇总」这个代理更硬 —— 汇总那道闸是给不点名的那一大批用的。
  //
