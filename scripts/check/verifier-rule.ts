 ...
 *
 * 只扫 `import` 收到的比三种边全收少一大截。**两个数都不写在这儿** —— 它们随仓库动，
 * 而没有任何检查看着这段注释（同一段里的两个数曾经隔四行各说各的，ADR-82 记了）。
 * 要看现在各是多少，照 ADR-82 里那条命令跑 —— **命令写在记录里、不写在这儿**，
 * 因为抽边那条判据连注释里提到的路径也收：在这段注释里写一个
 * 形如模块路径的字面量，会把它自己加进闭包（实测加进去的还是一个拼歪了的、
 * 不存在的路径）。
 *
 * 少收的那几个里有 `mutate-rule.ts`（判定「抓到还是崩了」的那一半）——
 * 一条打在它身上的 `by: "selfcheck"` 变异，改的正是给它自己判分的那把尺。
 *
 * 闭包里包含**本文件自己** —— 自检 `import` 它来拿 `SELFCHECK_TOOLS`，所以这条判据
 * 也在它自己划的禁区里。这不是巧合：它就是验证基础设施。
 *
 * ## 它挡不住什么
 *
 * - **验证者从 `scripts/check/` 外面起一个工具**：按上面那条判据会被当成被测对象放行。
 *   今天没有这种写法（`SELFCHECK_TOOLS` 里的路径全在 `check/` 下 —— 那张表就在下面，
 *   一眼看得完），但判据拦不住有人这么写。
