 ...
 * | `runConformance()` | **真的** `Budget` 与 **真的** `TikHub.get()` | 真实实现，但只在有界的响应序列上 |
 *
 * 两半共用同一套协议：`explore()` 走 `step()` 的单步动作，`runConformance()` 走
 * `modelCall()` —— 后者是同一个协议在「一次 get() 调用」这个粒度上的投影，
 * 拿它和真实实现逐步对照。一份只有 `explore()` 的资产会是 `formal/` 目录里
 * 一个与实现无关的玩具。
 *
 * ── 不在模型里的东西 ──────────────────────────────────────────────
 *
