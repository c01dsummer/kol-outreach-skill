...
2. **IG 的发现路径与原设计不同。** 原计划用 hashtag 端点，实测它的 `owner` 只有
   ...
   在 IG 上的对应物），而**我们只从它取一页**，一个关键词约 12 条。
   ⚠️ 2026-09-21 更正：这一条原本写的是「它**没有分页游标**」。那是个**推论**，不是实测 ——
   证据只有「响应里只有 `count` 和 `items`」，而我们的请求从头到尾只发过 `keyword`
   一个参数，**没试过 `offset`／`max_id`**。加上第 3 条（OpenAPI 不描述响应体），
   当时这件事没有答案。
   ⚠️ 2026-09-22 真跑一次之后：**响应里有 `data.pagination_token`**，所以「没有游标」
   这个推论是错的；但那个值回传管不管用仍未试，而且这个端点**在漂**（两次相同请求
   返回不同条目集），所以候选参数一条都没判成。复跑 `npm run probe:ig-paging`（ADR-101）
