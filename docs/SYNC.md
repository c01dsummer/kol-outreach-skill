...
| **新增/修改一条验收判据** | `accept` 判据编号稳定 · `test.ts` 或 `scripts/check/selfcheck.ts` 里 `criterion(...)` 认领（**两种认领同权**；入口的接线缺省那个验证者够不到，只有自检发得出） · **只由自检认领的（单元没认），要配一条 `by: "selfcheck"` 的负片** ——缺省那个验证者够不到入口，不写 `by` 的变异只会「存活」 · 红线判据两种认领一个都没有时要显式登记豁免 | 🔒 `spec` · `audit` 逐条数：红线判据两种认领都没有是硬失败，只由自检认领却没有那种负片也是硬失败 |
...
| **改一份形式化模型或它的边界参数** | 两份模型都要跟上（`formal/budget/BudgetProtocol.tla` 与 `scripts/check/formal-rule.ts`）· 场景表里每条不变量的**预期反例长度**要照推导重算，不要跑一遍粘回来 · `formal/budget/IMPLEMENTATION-MAP.md` 的结论表与人工核对表 · `docs/ARCHITECTURE.md` 锚点表里那两行的说明列 · 守判定的那几条变异 | 🔒 `formal` 跑抽象模型穷举与真实实现对照；`audit` 要求判定模块有变异（**两份模型对不对得上要手动跑 `-- --tla`，检查链里没有**）|
