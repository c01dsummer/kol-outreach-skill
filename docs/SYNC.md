...
| **新增/修改一条验收判据** | `accept` 判据编号稳定 · `test.ts` 或 `scripts/check/selfcheck.ts` 里 `criterion(...)` 认领（**两种认领同权**） · **只由自检认领的（需求测试未认领），要配一条 `by: "selfcheck"` 的负片** · 红线判据两种认领一个都没有时要显式登记豁免 | 🔒 `spec` · `audit` 逐条数：红线判据两种认领都没有是硬失败，只由自检认领却没有那种负片也是硬失败 |
