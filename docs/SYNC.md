...
| **新增/修改一条验收判据** | `accept` 判据编号稳定 · `test.ts` 或 `scripts/check/selfcheck.ts` 里 `criterion(...)` 认领（**两种认领同权**；入口的接线缺省那个验证者够不到，只有自检发得出） · 红线判据两种认领一个都没有时要显式登记豁免 | 🔒 `spec` · `audit` 逐条数，红线判据两种认领都没有是硬失败 |
