...
| `scripts/check/spec-rule.ts` | 检查 | — | 需求登记表的**判定**：形状（根对象与每条需求：渲染要读的字段必须在，缺了会把空值写进生成的文档）、完整性（编号唯一、交点与决策记录编号真实存在、作废须有理由）、表格渲染与内容指纹（凡影响渲染的都进指纹，包括分类表及其顺序）、审计对一条需求的裁定（红线判据逐条认领；一条都没认领同样是缺口）；不碰文件系统，因而可被测 |
...
| `scripts/check/claims.ts` | 检查 | — | 测试运行时留下的覆盖记录（跑过哪些需求、认领了哪些验收判据）的路径与形状，以及关于它的五个判定：新鲜度（带整棵 `scripts/` 树的指纹，对不上就是过期记录）、形状（认不出就说认不出，不兜底成空数组；该验哪几栏列成一张名单，加字段时只改那一处）、归属（谁拥有这份记录，谁负责开跑前先清掉它）、写盘资格（干净、断言全过、而且源码一路没被动过的运行才写得下）、读不出来时是哪一种读不出来（还没跑过／记录坏了／这个路径读不了，说法不一样）。写的一方（`test.ts`）与读的一方（`audit.ts`）由此指向同一处 |
