...
| `scripts/check/spec-rule.ts` | 检查 | — | 需求登记表的**判定**：形状（根对象与每条需求：渲染要读的字段必须在，缺了会把空值写进生成的文档）、完整性（编号唯一、交点与决策记录编号真实存在、作废须有理由）、表格渲染与内容指纹（凡影响渲染的都进指纹，包括分类表及其顺序）、审计对一条需求与交点的裁定（红线判据逐条认领；一条都没认领同样是缺口；含红线的交点没认领是硬失败）；不碰文件系统，因而可被测 |
...
| `scripts/check/claims.ts` | 检查 | — | 测试运行时留下的覆盖记录的路径与形状，以及关于它的四个判定：新鲜度（带整棵 `scripts/` 树的指纹，对不上就是过期的，不算数 —— 只钉 `test.ts` 挡不住「被 import 的实现改坏、测试在清记录之前就崩了」那一路）、形状（缺字段、字段不是数组、数组里混进非字符串的记录一律认不出 —— 不兜底成空数组，那会把「记录坏了」说成「这些东西没测过」）、归属（谁拥有这份记录，谁负责开跑前先清掉它 —— 半路崩掉就没有记录，而不是留下上一次的当证据）、写盘资格（只有一次干净的运行才写得下 —— 变异运行与断言红过的运行都不算）。写的一方（`test.ts`）与读的一方（`audit.ts`）由此指向同一处 |
