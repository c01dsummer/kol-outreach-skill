...
| `scripts/check/verifier-rule.ts` | 检查 | — | 一条变异改的是不是**验证者自己要用的东西**（`by` 指名了验证者时才管）。验证者够得到一个文件有三条路：`import`、当工具起、以及靠 `--import` 预加载 —— 只扫第一条，闭包从 14 个缩到 3 个，而偏小的那一头是放行。划开「工具」与「被测对象」的判据是：起 `scripts/check/` 底下的算工具，起别处的算被测对象 —— 后者正是本条记录的头号用例（自检真跑被变异的入口），收进闭包就把它自己挡在门外了。**抽边故意写得宽**（注释里提到的路径也收）：两头不对称，多收只是把闭包撑大、多拦几条，少收就是放行一条自己验自己的变异。自检那句失败汇总的文案也收在这里 —— 自检打、判定认，只此一份 |
...
| 硬来收尾：先给每个 worker 壳一刀 → 再按组收掉各自的验证者 → 最后删隔离目录 | `scripts/check/jobs-rule.ts` | 删目录排在收验证者之前，那几组还能往一棵正在被删的树里写，盘上重新长出半成品，而「收干净了」那句已经说出口 —— 静默；丢掉按组那一半就回到「硬杀必漏一批跑着被改过源码的进程」 | M-H40-g M-H40-h |
