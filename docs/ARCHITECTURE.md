...
> 会，它就有语义，就该搬进 `lib/`（`docs/CONVENTIONS.md` 第 10 条 —— 这条踩过多次，每一次都记在那里）。
...
| `scripts/check/selfcheck-rule.ts` | 检查 | D6 | 上面那条自检里「**续跑的代价说清楚了没有**」的判定：免费和要花钱两句必须二选一，说的还得是**该说的那一句**（该说哪一句由这一轮实际还剩多少活定，说反了要拦），说了要花钱就得说出还剩什么，而且那个「还剩什么」不能是空的。抽出来是为了它能被测、能被变异 —— 留在入口里，ADR-22 那张空头支票的写法就从这条检查自己身上过去了，没有任何东西会红。同一处还管着这条自检里另外三条判定：收尾报的「还剩什么」**得点到 profile**（入口不把创作者交给算钱那一步，光剩关键词也照样不红）；预算用尽时给的**恢复命令得带 `--budget`**（不带的话照着敲下去立刻再撞同一个退出码）；**续跑得真的又发了请求**（bio 查到了、外链是空的那种人被漏在补全循环外面时，两次跑下来请求数一次都不涨） |
