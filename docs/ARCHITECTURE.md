...
| `scripts/check/audit.ts` | 检查 | — | 链路审计，回答完成度而不是「功能做完了没有」；计量单位是**验收判据**不是需求，红线的判据**两种认领一个都没有**（单元断言没认、自检也没端到端跑过）就硬失败；**检查链自己的判定模块**也在它的名单上，没有变异守着就硬失败 |
| `scripts/check/audit-rule.ts` | 检查 | — | 审计里三半**判定**。① 检查链自己算不算数：哪些文件是判定模块（scripts/check/ 下不带 shebang 的，不按文件名认）、哪些没有变异守着。② 计量输入怎么分：现行的参与计量、作废的只参与展示，两半一起给（只过滤不展示，作废的需求就从审计里彻底消失）。③ 报告上那几个数怎么数：哪些变异是判据级的、验收判据那一行里测试认领／入口认领／显式豁免**三栏互不重叠**（两边都认领的算在测试那一栏、只有自检认领的才进入口那一栏、豁免只数两边都没认领的 ——各数各的会把同一条数两遍，红线那三栏加出比总数还大的值，而逐条那一头早把它算成认领了）。抽出来都是为了能被测 —— 它自己也在①那张名单上 |
