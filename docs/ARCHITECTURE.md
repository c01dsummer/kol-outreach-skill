...
| `scripts/check/formal.ts` | 检查 | P3 F7 D6 | 把「钱不在 `charge()` 里花掉」这件事变成能报错的检查：跑模型、跑对照、打印、退出码。三档结论分开说 —— 抽象模型穷举 / 真实实现对照 / 环境不具备时说「无从判断」而不是放行 |
| `scripts/check/formal-rule.ts` | 检查 | P3 F7 D6 | 上面那条的**判定**：预算与请求提交协议的状态机（状态、动作、不变量、有界参数、每个场景**记下来的**预期反例长度）、宽度优先穷举、与 `formal/budget/BudgetProtocol.tla` 对账的规范写法，以及拿真实 `Budget`／`TikHub.get()` 逐步对照的驱动。抽出来是为了它能被测 —— **模型自己也会写错** |
