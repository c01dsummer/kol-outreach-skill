...
| `scripts/check/mutate-rule.ts` | 检查 | — | 上面那条的**判定**：一次运行算「抓到」「红错了地方」「跑不起来」还是「存活」—— 只有断言红了才算抓到（崩溃不是断言的功劳），而点了名的还要红在点名那一条上（红在别处，对那条夹具什么也没证明）。**认得哪些验证者也在这里**：跑哪个脚本、它的失败汇总长什么样，两样同一处声明 —— 分开放的话，换一边不换另一边的症状是「它真的红了，却被判成跑不起来」 |
