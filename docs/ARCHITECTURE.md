...
| `scripts/check/audit-rule.ts` | 检查 | — | 审计里三半**判定**。① 检查链自己算不算数：哪些文件是判定模块（scripts/check/ 下不带 shebang 的，不按文件名认）、哪些没有变异守着。② 计量输入怎么分：现行的参与计量、作废的只参与展示，两半一起给（只过滤不展示，作废的需求就从审计里彻底消失）。③ 报告上那几个数怎么数：哪些变异是判据级的、验收判据那一行里测试认领／入口认领／显式豁免三份名单各数各的（合成一个数就分不出证据与缺口）。抽出来都是为了能被测 —— 它自己也在①那张名单上 |
