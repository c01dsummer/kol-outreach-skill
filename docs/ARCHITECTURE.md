...
| `scripts/check/claims.ts` | 检查 | — | 测试运行时留下的覆盖记录的路径与形状，以及关于它的五个判定：新鲜度（带整棵 `scripts/` 树的指纹，对不上就是过期记录）、形状（认不出就说认不出，不兜底成空数组）、归属（谁拥有这份记录，谁负责开跑前先清掉它）、写盘资格（干净、断言全过、而且源码一路没被动过的运行才写得下）、读不出来时是哪一种读不出来（还没跑过／记录坏了／这个路径读不了，说法不一样）。写的一方（`test.ts`）与读的一方（`audit.ts`）由此指向同一处 |
