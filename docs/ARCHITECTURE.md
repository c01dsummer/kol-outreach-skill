...
| `scripts/check/claims.ts` | 检查 | — | 测试运行时留下的覆盖记录的路径与形状，以及关于它的四个判定：新鲜度（带 `test.ts` 指纹，对不上就是过期的，不算数）、形状（缺字段、字段不是数组的记录一律认不出 —— 不兜底成空数组，那会把「记录坏了」说成「这些东西没测过」）、归属（谁拥有这份记录，谁负责开跑前先清掉它 —— 半路崩掉就没有记录，而不是留下上一次的当证据）、写盘资格（只有一次干净的运行才写得下 —— 变异运行与断言红过的运行都不算）。写的一方（`test.ts`）与读的一方（`audit.ts`）由此指向同一处 |
