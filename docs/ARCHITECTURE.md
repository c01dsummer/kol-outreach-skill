...
| `scripts/check/spec-rule.ts` | 检查 | — | 需求登记表的**判定**：形状（根对象与每条需求：渲染要读的字段必须在，缺了会把空值写进生成的文档）、完整性（编号唯一、交点与决策记录编号真实存在、作废须有理由）、表格渲染与内容指纹（凡影响渲染的都进指纹，包括分类表及其顺序）、审计对一条需求的裁定（红线判据逐条认领；一条都没认领同样是缺口）、审计对一个交点的裁定（认领编号两侧顺序无关；**有红线的交点没有测试认领就是硬失败**）；不碰文件系统，因而可被测 |
