...
| `scripts/check/claims.ts` | 检查 | — | 测试运行时留下的覆盖记录的路径与形状，以及关于它的两个判定：新鲜度（带 `test.ts` 指纹，对不上就是过期的，不算数）与写盘资格（只有一次干净的运行才写得下 —— 变异运行与断言红过的运行都不算）。写的一方（`test.ts`）与读的一方（`audit.ts`）由此指向同一处 |
