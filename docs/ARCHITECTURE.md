...
| `scripts/check/jobs-rule.ts` | 检查 | — | 变异跑的**派工**判定：派几个、一条结论怎么写成一行又怎么读回来、打断时刀往哪儿发、以及派出去却没回话的怎么算。并行的单位是**目录**不是进程 —— 一条变异是真的改写一份源文件，两条同时改同一棵树，验证者跑的就是「两处叠在一起」那棵树，而报告仍按条归因，每一条的绿或红都不算数。派法是**一条一派**不是先切好再发：指名了验证者的那几条跑的是另一个验证者，代价与缺省那个不可比（原型上量过，静态切成 6 份时两份比另外四份多跑 32 秒），而要靠静态切法摊平就得先知道每条要跑多久 —— 那个数没人量过，猜一个写进代码就是把「不知道」伪装成「知道」。**打断时两层都要按组杀**：起一个 `.ts` 要经过 `tsx` 的壳，而壳会再分出一个真正跑脚本的进程 —— SIGTERM 壳会转下去，SIGKILL 转不了（两条都实测过），刀落在壳上就是「壳没了，干活那个变成孤儿接着跑」；验证者又自成一组（那是 worker 能连它底下的孙子一起杀的前提），既收不到打在 worker 身上的那一刀、也收不到打在派工进程上的，所以它的组号要由 worker 报上来。组号 0 和 1 一律不发：`kill(-0)` 打的是自己这一组，`kill(-1)` 是这个用户有权打的每一个进程。**派出去却没有结论回来的算硬失败**：worker 崩了、被杀了、汇报行被截断，在输出上长得都一样，而把「没查过」当成「跑过了、没事」正是 `process/README.md` 总纲里那个头号事故 |
