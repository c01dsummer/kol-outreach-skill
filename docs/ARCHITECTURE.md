...
| `scripts/check/mutate.ts` | 检查 | — | 给测试做的测试：变异存活即那条测试是假的；验证者崩了不算抓到。缺省的验证者是 `scripts/test.ts`；写了 `by` 的那些改跑别的（今天只有自检），为的是接线那一层 —— 入口脚本里的接线 `test.ts` 够不到，长期只能靠显式缺口顶着。`by` 与 `kills` 同进同出 —— `kills` 点明该红的是哪一条夹具，而不写 `by` 只写 `kills` 虽然在机制上生效也照样拦下：那是 ADR-70 明写「另外评定」的那条延伸 |
...
| `scripts/check/verifier-rule.ts` | 检查 | — | 一条变异改的是不是**验证者自己要用的东西**（`by` 指名了验证者时才管）。验证者够得到一个文件有三条路：`import`、当工具起、以及靠 `--import` 预加载 —— 只扫第一条，闭包从 13 个缩到 3 个，而偏小的那一头是放行。划开「工具」与「被测对象」的判据是：起 `scripts/check/` 底下的算工具，起别处的算被测对象 —— 后者正是本条记录的头号用例（自检真跑被变异的入口），收进闭包就把它自己挡在门外了 |
