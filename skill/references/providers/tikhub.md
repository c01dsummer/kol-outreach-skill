...
1. **顺着续页令牌翻，页数随达标走**（D6.u、D6.v，ADR-111）。首页只发 `keyword`；之后带上一页响应交回的
   ...
   **2026-09-22 一次带令牌的链式请求获正常响应。** 参数名是 `pagination_token`
   ...
   npm run probe:ig-paging -- --keyword smoothie --chain 10   # 顺着游标请求，自带等次数重发曲线
   npm run probe:ig-paging -- --keyword smoothie --repeat 10  # 原样重发，只报本次观测
   ```

   ⚠️ **`--chain` 一定同时跑等长度原样重发曲线**，最多发 2N 次请求；缺令牌时两边
   一起提前停止。端点会漂，这两条曲线只能报告各自累计量和当次差值，不能据此把
   差值归因给游标或漂移，也不能推断统计显著性或建议继续付费。

   ⚠️ **只看人，别看条目。** 同一个人连发几条 Reels，条目在涨而一个新达人都没多给，
   本次已识别作者数不会因此增加。结论那句话自己会点出是哪一种。

   ⚠️ **它给不出「服务端只有这么多人」这个结论。** 末尾连着几次不涨，说的是
   「**这 N 次之内**没再涨」——「问完了」与「还没问够」在一次观测里不可区分。
   响应中未取得可用的下一个游标时，探针会停止追链；这只能证明本次没有可继续的游标，
   不能证明已穷尽全部匹配结果。探针结论只报告这一层观测，见 ADR-101 第十三节末尾更正。
...
| 分页 | ✅ `offset` + `has_more` | 端点声明 `pagination_token` 参数；历史带令牌请求获正常响应，增量因果未核实。采集器顺着令牌请求、页数随达标走，令牌只在一次运行内有效（ADR-111）。另外端点会漂，见上 |
