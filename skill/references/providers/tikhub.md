...
1. **我们只取一页**（约 12 条）。代码只发 `keyword` 一个参数，第 2 页起直接返回空，不白花请求。
   ...
   **但「有游标」不等于「能翻页」**，这一条现在仍然没有答案：那个值回传给服务端
   管不管用，没人试过。同一跑里 `count`／`offset`／`page`／`max_id` 四个参数**一条都没判成** ——
   因为**基线自己两次就不一样**（两次完全相同的请求返回了不同的条目集），探针按规矩
   全判「作废」。端点会漂这件事本身也是那次才知道的。
   ...
   ⚠️ 读输出时**先看 `reading` 那一行的第一句**：响应里有没有像游标的键，与四个参数的
   对比**无关**（那是对一份响应的直接观测），所以四个参数全判「作废」时它照样成立。
   2026-09-22 那一跑正是这样：判词全是「作废」，而游标就躺在同一份输出里。
   「作废」那一支还会报**基线漂移量** —— 拿它跟每一行的「基线没有的」比，
   漂移小而某一行大得多才值得再试，一个量级就是噪音。

   **下一步**（谁先拿到能出网的机器＋充过值的 key 谁做）：把 `pagination_token` 的值
   当参数回传，看能不能接着往下翻。
