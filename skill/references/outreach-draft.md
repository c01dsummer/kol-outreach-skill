...
> 相关需求：**S1**（不发信，只产出草稿）· **S2**（仅英语）· **P2**（不得编造产品信息）
...
而你在 Phase 04 已经记录了 `observed_content`、`work_evidence` 与 `natural_integration`。只用其中已核实的内容，不把品牌偏好写成已证实的产品功能。
...
**只写英语（S2）。** 首版不做多语言 —— 判断母语容易出错，非英语文案写得生硬比写英语更伤，日语敬语这类文化规约风险更高。

**第一句必须引用具体内容。** 从已记录的 `work_evidence` 里挑一条最能说明他内容方向的，具体到实际读到的标题、做法或观点，并写明证据来自搜索命中作品还是独立主页样本。只见文案时不能写成看过视频画面、审美或评论。写不出具体的第一句，说明评审证据不足 —— 回去补，不要用套话糊过去。
**例外是 `recent_posts` 缺席或是个空数组**（没问过他的作品，不是他没作品）：这时写不出不是你的问题，留占位符 `{他的某条具体内容}` 让用户去主页看过再填 —— **不要编**。
...
本项目的 `brand_calibration` 可提示语气和自然场景；其中 `kind=brand_preference` 的条目只约束表达方式，不能当作产品事实。产品动作、规格及承诺须能指回 `kind=verified_product_fact` 的来源，未证实则保留占位符。草稿写入任务级 `agent-review.json`；导出时从同一评审投影，不因人工反馈改变草稿原文。
