...
| 文件读不出来 | 可能记着一百个人，**我读不出来** | `collect` 以退出码 2 结束，**不产出名单** |

「读不出来」有四种，**第三种更常见也更阴**：

1. **读不到** —— 权限不足、父路径不是目录、磁盘 IO 错。文件本身可能好好的，
   是**拿不到它**。报错里会带上真实原因，**照着那句去查，别默认是 JSON 坏了**
2. **解析失败** —— 少个逗号、写到一半被打断，一眼看得出坏了
3. **结构不对** —— 合法 JSON，但形状错了。最典型的是把 `creators` 的花括号
   改成方括号，或者删掉某个人的 `contacted`。它**能解析**，于是旧版本会
   若无其事地报「读到了」，然后一个人都不过滤
4. **键不能用** —— 键必须是 `platform:handle`。少个冒号，或者两个键
   （比如 `tiktok:Alice` 和 `tiktok:alice`）指向同一个人、各有一条记录，
   都算读不出来
